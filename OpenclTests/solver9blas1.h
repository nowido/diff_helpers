#ifndef SOLVER9BLAS1_H
#define SOLVER9BLAS1_H
...
struct Tile
{
    ...
    {
        size_t runStop = tileRows / SSE_BASE_COUNT;
        ...
        for(; row < runStop; row += SSE_BASE_COUNT, pScan += SSE_BASE_COUNT)
        {
            align_as(SSE_ALIGNMENT) float buf[SSE_BASE_COUNT];
            *(__m128*)buf = *(__m128*)pScan;

            for(size_t i = 0; i < SSE_BASE_COUNT; ++i)
            {
                float fav = fabs(buf[i]);
                bool change = (fav > maxValue);
                maxValue = change ? fav : maxValue;
                pivotIndex = change ? (row + i) : pivotIndex;     
            ...
            /*
            float fav = fabs(buf[0]);
            bool change = (fav > maxValue);
            maxValue = change ? fav : maxValue;
            pivotIndex = change ? row : pivotIndex;

            fav = fabs(buf[1]);
            change = (fav > maxValue);
            maxValue = change ? fav : maxValue;
            pivotIndex = change ? (row + 1) : pivotIndex;             

            fav = fabs(buf[2]);
            change = (fav > maxValue);
            maxValue = change ? fav : maxValue;
            pivotIndex = change ? (row + 2) : pivotIndex;                         

            fav = fabs(buf[3]);
            change = (fav > maxValue);
            maxValue = change ? fav : maxValue;
            pivotIndex = change ? (row + 3) : pivotIndex;                                     
            */
        }

        for(; row < tileRows; ++row, ++pScan)
        {
            float fav = fabs(*pScan);
            bool change = (fav > maxValue);
            maxValue = change ? fav : maxValue;
    ...
    int updateWithScaleAndNextPivotSearch1(size_t offsetVert, size_t offsetHoriz, const float* leadRowBlock)
    {
        ...
        if(tileRows == TILE_SIDE)        
        {
            ...
            for(; i < runStart; ++i)
            {
                ...
                bool change = (fav > maxValue);
                maxValue = change ? fav : maxValue;
                pivotIndex = change ? i : pivotIndex;                                     
            }
            
            __m128 qLdRowValue = _mm_set1_ps(ldRowValue);

            for(; i < tileRows; i += SSE_BASE_COUNT)
            {   
                float* p = column + i;
                
                _mm_store_ps(p, _mm_sub_ps(_mm_load_ps(p), _mm_mul_ps(_mm_load_ps(leadCol + i), qLdRowValue)));
                
                for(size_t j = 0; j < SSE_BASE_COUNT; ++j)
                {                    
                    float v = p[j];
                    float fav = fabs(v);
                    bool change = (fav > maxValue);
                    maxValue = change ? fav : maxValue;
                    pivotIndex = change ? (i + j) : pivotIndex;                                                     
                }
            }
        }
        else
