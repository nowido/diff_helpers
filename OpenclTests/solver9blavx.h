...
struct Tile
{
        ...
        tileCols(argTileCols)
    {
        ...
        runStop = tileRows / AVX_BASE_COUNT;
    ...
    void updateRowMajor()
    {
        ...
        {            
            ...
            {
                for(size_t col = 0; col < tileCols; col += AVX_BASE_COUNT)
                {
                    align_as(AVX_ALIGNMENT) float rowData[AVX_BASE_COUNT];

                    rowData[0] = pSrc[col * tileRows];
                    rowData[1] = pSrc[(col + 1) * tileRows];
                    rowData[2] = pSrc[(col + 2) * tileRows];
                    rowData[3] = pSrc[(col + 3) * tileRows];
                    rowData[4] = pSrc[(col + 4) * tileRows];
                    rowData[5] = pSrc[(col + 5) * tileRows];
                    rowData[6] = pSrc[(col + 6) * tileRows];
                    rowData[7] = pSrc[(col + 7) * tileRows];

                    _mm256_store_ps(pDest + col, *(__m256*)rowData);
                }    
            }
        }
        else
    ...
    void subtractProduct(const Tile* left, const Tile* top)
    {
        ...
        size_t innerBlocksCount = topTileRows / AVX_BASE_COUNT;
        ...
        if(tileRows == TILE_SIDE)
        {
            align_as(AVX_ALIGNMENT) float blockOfSums[AVX_BASE_COUNT];  
            
            for(size_t col = 0; col < tileCols; ++col, colData += topTileRows)        
            {       
                float* rowData = left->rowMajor;
                
                for(size_t row = 0; row < tileRows; row += AVX_BASE_COUNT, pDest += AVX_BASE_COUNT)
                {                        
                    ...
                    blockOfSums[4] = dotProduct(rowData, colData, innerBlocksCount);
                    rowData += leftTileCols;

                    blockOfSums[5] = dotProduct(rowData, colData, innerBlocksCount);                    
                    rowData += leftTileCols;

                    blockOfSums[6] = dotProduct(rowData, colData, innerBlocksCount);
                    rowData += leftTileCols;

                    blockOfSums[7] = dotProduct(rowData, colData, innerBlocksCount);                    
                    rowData += leftTileCols;
                                                         
                    _mm256_store_ps(pDest, _mm256_sub_ps(*(__m256*)pDest, *(__m256*)blockOfSums));                                        
                }
            }
        }
        else
...
private:
    ...
    inline float dotProduct(const float* rowData, const float* colData, size_t blocksCount)
    {
        ...
        {
            t = _mm256_mul_ps(blockRowData[i], blockColData[i]);
