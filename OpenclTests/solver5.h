...
struct Solver
{
    ...
    bool Init(size_t useDimension, ClPlayground* useCl, fn_cl_notify notify = NULL)
    {
        ...
        program = cl->PrepareProgram(dev, devco, "#include \"kern_eliminator5.cl\"");
        ...
        workGroupSizeY = 14; // to do: use preferredWorkGroupSizeMultiple

        runLength = workGroupSizeY * 5;
    ...
    void Solve()
    {
        ...
        clSetKernelArg(eliminatorKernel, 4, sizeof(cl_uint), &runLength);
        clSetKernelArg(eliminatorKernel, 5, runLength * sizeof(float), NULL);     
        ...
        const size_t minGridSquare = 128 * 128;
        ...
        {               
            ...
            if(workRectWidth * workRectHeight < minGridSquare)
            {
                ...
                // count "vertical" items needed to cover work rectangle with blocks of runLength

            size_t remWidth = workRectWidth % runLength;
            workSize[0] = (workRectWidth / runLength) + ((remWidth == 0) ? 0 : 1);
            ...
            clSetKernelArg(eliminatorKernel, 6, sizeof(cl_mem), pp + ppInputIndex);
            clSetKernelArg(eliminatorKernel, 7, sizeof(cl_mem), pp + ppOutputIndex);
